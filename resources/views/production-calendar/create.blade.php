...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="page-header">
        ...
        <div>
            <h1 style="font-size: 1.5rem; font-weight: 700; margin: 0;">Create Production Calendar (Bulanan)</h1>
            <p style="margin: 0; opacity: 0.9; font-size: 0.875rem;">Buat jadwal produksi bulanan</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="form-container">
        <form method="POST" action="{{ route('production-calendar.store') }}">
            @csrf
            ...
            <div class="card-section">
                <div class="form-grid-2">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="year" id="year" value="{{ now()->year }}" class="form-input" min="2000" max="2100" required>
                        @error('year')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="card-section">
                <div class="form-grid-3">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="total_days" id="total_days" min="1" max="31" class="form-input" placeholder="Otomatis terisi" required>
                        @error('total_days')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="planned_running_days" id="planned_running_days" min="0" class="form-input" placeholder="0" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="planned_shutdown_days" id="planned_shutdown_days" min="0" class="form-input" placeholder="0" required>
            ...
            <div class="action-buttons">
                ...
                <button type="submit" class="btn-red">
                    <span>💾</span>
                    <span>Simpan Production Calendar</span>
                </button>
            </div>
...
<script>
    $(document).ready(function() {
        // Inisialisasi Select2 untuk dropdown yang membutuhkan
        $('#area').select2({
        ...
        // Auto-calculate total days when year or month changes
        $('#year, #month').on('change', function() {
            calculateTotalDays();
        });

        // Auto-calculate planned_shutdown_days when total_days or planned_running_days changes
    ...
    // Ketika Plant dipilih, load Area
    ...
    // Function to calculate total days in selected month
    function calculateTotalDays() {
        const year = document.getElementById('year').value;
        const month = document.getElementById('month').value;
        
        if (year && month) {
            // Create date object and get days in month
            const daysInMonth = new Date(year, month, 0).getDate();
            document.getElementById('total_days').value = daysInMonth;
            
            // Recalculate planned shutdown after total days changes
            calculatePlannedShutdown();
        }
    }

    // Function to auto-calculate planned shutdown days
    function calculatePlannedShutdown() {
        ...
        if (totalDays > 0 && runningDays <= totalDays) {
            const shutdownDays = totalDays - runningDays;
            document.getElementById('planned_shutdown_days').value = shutdownDays;
        }
    }

    // Validate that running days doesn't exceed total days
